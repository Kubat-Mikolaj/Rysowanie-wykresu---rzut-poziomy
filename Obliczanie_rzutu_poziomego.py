# Program określający Rzut Poziomy.
# Wynikiem ma być wykres i określenie prędkości oraz wysokości
...
def read_datas():
    ...
    while h_start is None:
         h_start = float_input("Brak poprawnej wartości dla h_start. Typ float (np: 3.14)",
                               "Teraz podaj wysokość początkową (w m, min. 10):", 10,)

    while v_start is None:
             v_start = float_input("Brak poprawnej wartości dla v_start. Typ float (np: 3.14)",
                                    "Teraz podaj prędkość początkową (w m/s, min. 2):", 2, )

    return (h_start, v_start)

initial_values = None
while initial_values is None:
    print("Prosze, podaj danę niezbędne do wygenerowania wykresu.")
    initial_values = read_datas()

print(f"Dane początkowe wczytane ")
...
plt.plot(x_points, y_points, marker = ".", color = "blue", label = "Kolejne punkty rzutu.")
plt.grid()
plt.title(title)
plt.legend()
plt.xlabel("Odległość w metrach")
plt.ylabel("Wysokosc w metrach")
plt.show()